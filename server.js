...
app.post('/addfavourite', function(req, res) {
  ...
  db.collection('people').findOne({"login.username":req.session.user.login.username}, function(err, result) {
    db.collection("people").update({"_id": result._id},{$push:{"favourites" : {"place":{"name":req.body.resultHead, "address":req.body.resultPara, "category":req.body.categorySelect.innerHTML }}}});
  });
